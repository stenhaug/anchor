...
```{r achievegap, fig.cap = 'Target ability achievement gap estimates for each method.', out.width="70%", warning = FALSE, message = FALSE}
what %>% 
  mutate(
    method = 
      factor(
        ...
        labels = c("AOAA-OAT", "CLUSTER", "GINI", "AOAA-AS", "AOAA", "MINBC")
      )
  ) %>% 
...
```{r anchorfalse, fig.cap = 'Mean false anchor rates across 100 runs for each number of DIF items', out.width="70%", warning = FALSE, message = FALSE}
    ...
    mutate(
      method = 
        factor(
          ...
          labels = c("AOAA-OAT", "CLUSTER", "AOAA-AS", "AOAA")
        )
    ) %>% 
...
```{r anchorhit, fig.cap = 'Mean anchor hit rates across 100 runs for each number of DIF items', out.width="70%", warning = FALSE, message = FALSE}
temp %>%
    filter(true_anchor == "no dif") %>%
    mutate(rate = mean_selected / total) %>% 
    mutate(
