...
Figure \ref{fig:workplease} shows these distributions in the EMLG. 
...
Strobl used the Gini coefficient

Also the method where we say okay we can always add 1 to group ability and subtract 1 from group difference so we need a way to select from all of these solutions which are equivalent from a likelihood perspective. An obvious solution is to min total between curves

- min between curves perform similarly to minimizing l1 norm in Rasch case but has clearer solution for 2PL and on
